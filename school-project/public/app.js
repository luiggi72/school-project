...
function updateUIForLogout() {
    ...
    // Also reset any session state if needed
}
...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            alert('Error de conexión: ' + error.message + '. Verifica la consola.');
        }
    });
}
    ...
    navGroupHeaders.forEach(header => {
        header.addEventListener('click', () => {
            ...
            if (content) {
                ...
                // document.querySelectorAll('.nav-group-content').forEach(c => {
...
async function selectStudentForPayment(student) {
    ...
    if (!isSameFamily && currentPaymentItems.length > 0) {
        ...
        const confirmSwitch = confirm('Al cambiar de familia se borrarán los ítems actuales del carrito. ¿Deseas continuar?');
        if (!confirmSwitch) return; // Abort switch
...
if (btnDeleteSelected) {
    btnDeleteSelected.addEventListener('click', async () => {
        ...
        if (confirm(`¿Estás seguro de eliminar ${ids.length} solicitud(es)?`)) {
            try {
...
function renderLevelsList(levels) {
    ...
    levels.forEach(l => {
        const item = createAcademicItem(
            l.id, l.name, 'level',
            async (id) => {
                if (confirm('¿Eliminar Nivel? Se borrarán todos sus grados y grupos.')) {
...
async function loadGradesForLevel(levelId) {
    ...
    grades.forEach(g => {
        const item = createAcademicItem(
            g.id, g.name, 'grade',
            async (id) => {
                if (confirm('¿Eliminar Grado? Se borrarán todos sus grupos.')) {
...
function renderAreasList(areas) {
    ...
    areas.forEach(a => {
        const item = createAcademicItem( // Reusing the helper from Academic logic
            a.id, a.name, 'area',
            async (id) => {
                if (confirm('¿Eliminar Área? Se borrarán todas sus subáreas y puestos.')) {
...
async function loadSubareasForArea(areaId) {
    ...
    subareas.forEach(s => {
        const item = createAcademicItem( // Reusing helper
            s.id, s.name, 'subarea',
            async (id) => {
                if (confirm('¿Eliminar Subárea? Se borrarán todos sus puestos.')) {
...
function initPermissionsUI() {
    ...
    async function deleteRole(roleKey) {
        if (!window.currentRolesConfig) return;
        if (!confirm('¿Estás seguro de eliminar el rol "' + window.currentRolesConfig[roleKey].name + '"?')) return;
...
function switchParentsTab(tabId) {
    ...
    } else if (tabId === 'notifications' && activeParentStudent) {
...
// Render Notifications Info (Mock Data)
function renderStudentNotifications(student) {
    ...
    // Mock Data
    const notifications = [
        { type: 'payment', title: 'Recordatorio de Pago', message: 'La colegiatura de Diciembre vence en 3 días.', date: 'Hace 2 horas', read: false },
        { type: 'school', title: 'Suspensión de Clases', message: 'El próximo viernes 13 no habrá clases por consejo técnico.', date: 'Ayer', read: false },
        { type: 'academic', title: 'Nueva Calificación', message: 'Se ha publicado la calificación de Matemáticas del 2do Bimestre.', date: '05 Dic', read: true },
        { type: 'event', title: 'Festival Navideño', message: 'Invitamos a todos los padres al festival el 18 de Diciembre a las 10:00 AM.', date: '01 Dic', read: true }
    ];
    ...
    container.innerHTML = `
        <div style="max-width: 900px; margin: 0 auto; animation: fadeIn 0.3s ease;">
            ...
            <div style="margin-bottom: 2rem; border-bottom: 1px solid #e2e8f0; padding-bottom: 1rem; display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <h2 style="color: #1e293b; margin: 0; display: flex; align-items: center; gap: 0.75rem;">
                        <span class="material-icons-outlined" style="color: #6366f1;">notifications</span>
                        Notificaciones
                    </h2>
                    <p style="color: #64748b; margin: 0.5rem 0 0 2.25rem;">Avisos importantes de la escuela</p>
                </div>
                <button style="padding: 0.5rem 1rem; background: white; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.85rem; font-weight: 500; color: #475569; cursor: pointer;">
        ...
        let icon, color, bg;
        switch (note.type) {
            case 'payment': icon = 'payments'; color = '#ef4444'; bg = '#fef2f2'; break;
            case 'school': icon = 'school'; color = '#3b82f6'; bg = '#eff6ff'; break;
            case 'academic': icon = 'auto_stories'; color = '#22c55e'; bg = '#f0fdf4'; break;
            default: icon = 'campaign'; color = '#8b5cf6'; bg = '#f5f3ff';
        }

        return `
                        <div style="
                            background: white; 
                            border: 1px solid ${note.read ? '#e2e8f0' : '#cbd5e1'}; 
                            ...
                            box-shadow: ${note.read ? 'none' : '0 4px 6px -1px rgba(0, 0, 0, 0.05)'};
                        ...
                        ">
                            ...
                            <div style="flex: 1;">
                                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.25rem;">
                                    <h4 style="margin: 0; color: #1e293b; font-size: 1rem; font-weight: ${note.read ? '500' : '700'};">
                                        ${note.title}
                                        ${!note.read ? '<span style="display: inline-block; width: 8px; height: 8px; background: #ef4444; border-radius: 50%; margin-left: 0.5rem;"></span>' : ''}
                                    </h4>
                                    <span style="font-size: 0.8rem; color: #94a3b8;">${note.date}</span>
                                </div>
                                <p style="margin: 0; color: #475569; font-size: 0.95rem; line-height: 1.5;">${note.message}</p>
                            </div>
                        </div>
                    `;
    }).join('')}
...
// Parents Navigation
document.querySelectorAll('.nav-btn-parent').forEach(btn => {
    btn.addEventListener('click', () => {
        // Toggle active state
        document.querySelectorAll('.nav-btn-parent').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');

        // Toggle active style color
        document.querySelectorAll('.nav-btn-parent').forEach(b => b.style.color = '#94a3b8');
        btn.style.color = 'var(--primary)';

        // Show section
        const targetId = btn.getAttribute('data-target');
        document.getElementById('parents-home-section').classList.add('hidden');
        document.getElementById('parents-finance-section').classList.add('hidden');
